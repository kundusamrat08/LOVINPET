...
if (isset($_POST['add_to_cart'])) {
    if (isset($_SESSION['email'])) {
        ...
        $usertype = $_POST['User_Type'];    
            ...
            $sql_insert_cart = "INSERT INTO cart (user_id, user_name, product_id, product_category, product_name, image, acc_type, stock, price, User_Type, email, quantity) VALUES ('$user_id', '$username', '$product_id', 'Food', '$product_name', '$product_image', '$product_type', '$product_stock', '$product_price', '$usertype', '$user_email', 1)";
...
if (isset($_POST['buy_now'])) {
    ...
    $sql_insert_cart = "INSERT INTO cart (user_id, user_name, product_id, product_category, product_name, image, acc_type, stock, price, User_Type, email,quantity,buy_now) VALUES ('$user_id', '$username', '$product_id', 'Food', '$product_name', '$product_image', '$product_type', '$product_stock', '$product_price', '$usertype', '$user_email', 1,'bn')";
...
if(isset($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $product_cate = $row['food_for_pet'];
        $product_type = $row['food_unit'];

        $product_description = $row['food_description'];
            ...
            <div class="product-div-right">
                ...
                <div class="btn-groups">
                    <form method="post" action="">
                        ...
                        <input type="hidden" name="product_name" value="<?php echo $row['food_name']; ?>">
                        <input type="hidden" name="image" value="<?php echo $row['image1']; ?>">
                        <input type="hidden" name="type" value="<?php echo $row['food_unit']; ?>">
                        <input type="hidden" name="stock" value="<?php echo $row['food_stock']; ?>">
...
<div class="related-products">
    ...
    <div class="product-row">
        ...
        $sql_related = "SELECT * FROM food WHERE id != $product_id AND status = 'Activate' ORDER BY RAND() LIMIT 4";
        ...
        if ($result_related->num_rows > 0) {
            while ($row_related = $result_related->fetch_assoc()) {
                ...
                echo "<a class='id-txt' href='food_details2.php?id=" . $row_related['id'] . "'>";
                // Display the first image from the related products
                echo "<img src='upload/" . $row_related['image1'] . "' alt='" . $row_related['food_name'] . "'>";
                echo "<h3 class='name-txt'>" . $row_related['food_name'] . "</h3>";
                echo "<p class='price-txt'>₹" . $row_related['food_price'] . "</p>";
                echo "</div>";
            }
        } else {
