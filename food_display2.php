...
$sql = "SELECT id, food_name, food_for_pet, food_unit, food_price, food_stock, image1, food_description, User_Type FROM food WHERE status='Activate'";


if(isset($_GET['category']) && !empty($_GET['category'])) {
    $category = mysqli_real_escape_string($conn, $_GET['category']);
    $sql .= " AND food_for_pet='$category'";
}
...
if(isset($_GET['price']) && !empty($_GET['price'])) {
    ...
    $sql .= " AND food_price BETWEEN $min_price AND $max_price";
}
    ...
    if (isset($_SESSION['email'])) {
        ...
        if ($result_user && mysqli_num_rows($result_user) == 1) {
            ...
            if (mysqli_query($conn, $sql_insert_cart)) {
                header("Location: food_display2.php");
            } else {
...
<head>
    ...
    <link rel="stylesheet" href="temp/food_display.css">
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<a class='id-txt' href='food_details2.php?id=" . $row['id'] . "' target='_blank'>";
        echo "<div class='product-content'>";
        echo "<img class='imgs' src='upload/" . ($row['image1'] ?? '') . "' alt='Product Image'>";
        echo "<h3>" . ($row['food_name'] ?? '') . "</h3>";
        echo "<p class='category-txt'>Category: " . ($row['food_for_pet'] ?? '') . "</p>";
        echo "<p class='type-txt'>" . ($row['food_unit'] ?? '') . "</p>";
        echo "<p class='price-txt'>Price: ₹" . ($row['food_price'] ?? '') . "</p>";
        // echo "<p>Description: " . ($row['about_accessories'] ?? '') . "</p>";
        // $userType = isset($row['User_Type']) ? $row['User_Type'] : ''; 
        // Form for adding to cart
        echo "<form method='post' action=''>";
        echo "<input type='hidden' name='product_id' value='" . $row['id'] . "'>";
        echo "<input type='hidden' name='product_name' value='" . $row['food_name'] . "'>";
        echo "<input type='hidden' name='image' value='" . $row['image1'] . "'>";
        // Add other hidden fields if needed
        echo "<input type='hidden' name='type' value='" . $row['food_unit'] . "'>";
        // Add other hidden fields if needed
        echo "<input type='hidden' name='stock' value='" . $row['food_stock'] . "'>";
        echo "<input type='hidden' name='price' value='" . $row['food_price'] . "'>";
