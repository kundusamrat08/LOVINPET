...
$sql = "SELECT id, medicine_name, medicine_pet, medicine_unit, medicine_price,  medi_stock, image1, medi_desc, User_Type FROM medicine WHERE status='Activate'";


if(isset($_GET['category']) && !empty($_GET['category'])) {
    $category = mysqli_real_escape_string($conn, $_GET['category']);
    $sql .= " AND medicine_pet='$category'";
}
...
if(isset($_GET['price']) && !empty($_GET['price'])) {
    ...
    $sql .= " AND medicine_price BETWEEN $min_price AND $max_price";
}
...
if (isset($_POST['add_to_cart'])) {
    ...
    // echo "<p><br><br><br><br><h1>You need to Login first!</h1><br><a href='login.php'>Login Now!</a><br><br><br><br><br><br></p>";
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<a class='id-txt' href='medicine_details.php?id=" . $row['id'] . "' target='_blank'>";
        echo "<div class='product-content'>";
        echo "<img class='imgs' src='upload/" . ($row['image1'] ?? '') . "' alt='Product Image'>";
        echo "<h3>" . ($row['medicine_name'] ?? '') . "</h3>";
        echo "<p class='category-txt'>Category: " . ($row['medicine_pet'] ?? '') . "</p>";
        echo "<p class='type-txt'>" . ($row['medicine_unit'] ?? '') . "</p>";
        echo "<p class='price-txt'>Price: ₹" . ($row['medicine_price'] ?? '') . "</p>";
        // echo "<p>Description: " . ($row['about_accessories'] ?? '') . "</p>";
        // $userType = isset($row['User_Type']) ? $row['User_Type'] : ''; 
        // Form for adding to cart
        echo "<form method='post' action=''>";
        echo "<input type='hidden' name='product_id' value='" . $row['id'] . "'>";
        echo "<input type='hidden' name='product_name' value='" . $row['medicine_name'] . "'>";
        echo "<input type='hidden' name='image' value='" . $row['image1'] . "'>";
        // Add other hidden fields if needed
        echo "<input type='hidden' name='type' value='" . $row['medicine_unit'] . "'>";
        // Add other hidden fields if needed
        echo "<input type='hidden' name='stock' value='" . $row['medi_stock'] . "'>";
        echo "<input type='hidden' name='price' value='" . $row['medicine_price'] . "'>";
